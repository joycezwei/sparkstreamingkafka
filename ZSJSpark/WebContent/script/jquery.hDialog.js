	...
	methods = {
        init: function (options) {
           	return this.each(function() {
				...
				T.data('hDialog', _O);
				_O.autoShow ? methods.open(_O,T) : T.on('click',function() { var sid = _O.box.substring(1,_O.box.length-4);var id = getIdSelections(sid,'id'); if(id.length==1||_O.type==0){methods.open(_O,T);} });
//				_O.autoShow ? methods.open(_O,T) : T.on('click',function() { methods.open(_O,T); });
        ...
        open: function (o,T) {
			...
			//弹框标题和关闭按钮
			headTpl = (o.title != '' && $('#HTitle').length <= 0) ? '<div id="HTitle">'+o.title+'</div> <a class="HCloseBtn" title="关闭"><span>&times;</span></a>' : '';
