package com.st.zsjspark.controller;

import java.util.List;
import java.util.Map;

import javax.annotation.Resource;

import net.sf.json.JSONObject;

import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseBody;

import com.st.zsjspark.comn.util.Tools;
import com.st.zsjspark.model.Machine;
import com.st.zsjspark.service.IMachineService;

@Controller
@RequestMapping("/MachineController")
public class MachineController {
	
	@Resource
	private IMachineService machineService;
	
	@RequestMapping(value = "/addNew.action", produces = "text/html;charset=UTF-8")
	@ResponseBody
	public  String insertNewRecord(Machine record){
		record.setStatus(2);
		record.setCreateDate(System.currentTimeMillis());
		machineService.insert(record);
		
		JSONObject json = new JSONObject();
		json.put("error_code", 200);
		return json.toString();
	}
	
	@RequestMapping(value = "/update.action", produces = "text/html;charset=UTF-8")
	@ResponseBody
	public String updateRecord(Machine record){
		record.setChangedDate(System.currentTimeMillis());
	    machineService.update(record);
	    JSONObject json = new JSONObject();
		json.put("error_code", 200);
		return json.toString();
	}
	
	@RequestMapping(value = "/getbyId.action", produces = "text/html;charset=UTF-8")
	@ResponseBody
	public String getRecordbyId(int id){
	
		Machine record = machineService.findById(id);
		JSONObject json = new JSONObject();
		json.put("data", record);
		json.put("error_code", 200);
		return json.toString();
	}
	
	@RequestMapping(value = "/delete.action", produces = "text/html;charset=UTF-8")
	@ResponseBody
	public String deleteRecord(Machine record){
		record.setDeleteDate(System.currentTimeMillis());
		machineService.update(record);
	    JSONObject json = new JSONObject();
		json.put("error_code", 200);
		return json.toString();
	}
	
	@RequestMapping(value = "/conditionSearch.action", produces = "text/html;charset=UTF-8")
	@ResponseBody
	public String findbyCondition(Machine record){
		Map map = Tools.transBean2Map(record);
		List<Machine> list = machineService.findByCondition(map);
	    JSONObject json = new JSONObject();
	    json.put("data", list);
//	    json.put("rows", list);
//    json.put("total", 15);
		return json.getString("data");
	}
	
	@RequestMapping(value = "/getMachineStatus.action", produces = "text/html;charset=UTF-8")
	@ResponseBody
	public String getMachineStatus(){
		Map map = machineService.getMachineStatus();
		JSONObject json = new JSONObject();
		json.put("data", map);
		return json.getString("data");
	}
	
	@RequestMapping(value = "/findMachine.action", produces = "text/html;charset=UTF-8")
	@ResponseBody
	public String findMachine(Machine record){
		Map map = Tools.transBean2Map(record);
		List<Machine> list = machineService.findMachine(map);
    JSONObject json = new JSONObject();
	    json.put("data", list);
	    json.put("rows", list);
	    json.put("total", 15);
		return json.getString("data");
	}
}
